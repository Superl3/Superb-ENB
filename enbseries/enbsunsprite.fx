...
float  Glare_Intensity    <string UIName="Glare Intensity";                      float UIMin=0; > = {0.1};  //glare intensity
float  Glare_Scale        <string UIName="Glare Scale";                          float UIMin=0; > = {1.5};  //glare scale
...
float4  PS_Glare( VS_OUTPUT_POST IN, float2 vPos : VPOS) : COLOR
{
    ...
    float  sunmask = pow(tex2D(SamplerMask, 0.5).x, 3.0) * Glare_Intensity;
    clip(  sunmask - 0.002);//skip current pixel if sunmask < 0.002
...
//blinking ends
    ...
    res.rgb *= Glare_Tint * sunmask * LightParameters.w; //weight, tint
    return res;
}
